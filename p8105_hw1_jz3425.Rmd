...
This dataset includes data of `r nrow(penguins)` penguins and these `r ncol(penguins)` variables of them: `r names(penguins)`.

Among all the penguins, there are `r nlevels(pull(penguins, names(penguins[1])))` kinds of `r names(penguins[1])`: `r levels(pull(penguins, names(penguins[1])))`, coming from `r nlevels(pull(penguins, names(penguins[2])))` different `r names(penguins[2])`: `r levels(pull(penguins, names(penguins[2])))`.
